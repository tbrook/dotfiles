...
// 
key.setGlobalKey([["<f1>", "b"], ["C-z", "k"]], function (ev) {
                key.listKeyBindings();
            }, 'キーバインド一覧を表示', false);
...
// https://gist.github.com/shinnya/463159
key.setViewKey(["C-z", "u", "u"], function (ev, arg) {
    display.echoStatusBar("Copy URL to clipboard");
    command.setClipboardText(getBrowser().contentDocument.URL);
}, 'Copy URL to clipboard');
  
  
// http://k9q.hateblo.jp/entry/2014/07/13/224049
key.setViewKey(["C-z", 'u', 'a'], function (ev, arg) {
    var tabs = gBrowser.mTabContainer;
    var txt = "";
    for( var i = 0; i < tabs.itemCount; i++ ) {
        var tab   = tabs.childNodes[i];
        var title = tab.label;
        var uri   = tab.linkedBrowser.contentWindow.location.href;
        var txt   = txt + "-\n" + "  uri: " + uri + "\n" + "  title: " + title + "\n";
    }
